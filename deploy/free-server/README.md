...
| 方案                               | 免费额度                                                                     | 大陆访问         | 限制                            | 适合                  |
| -------------------------------- | ------------------------------------------------------------------------ | ------------ | ----------------------------- | ------------------- |
| **Oracle Cloud Always Free** ⭐推荐 | **永久免费**：ARM A1 最高 4 核 / 24GB + 200GB 磁盘 + 10TB / 月流量（或 AMD E2.1 微实例 ×2） | 好（选韩国 / 日本区） | 注册需信用卡预授权（不扣费）；长期低负载可能被回收     | 想要真正的免费 VPS，可长期稳定运行 |
| 国内免费云（阿贝云 / 三丰云类）                | 免费 VPS 1 核 / 1G / 独立 IP                                                  | 最好           | 需实名认证；小厂稳定性与信誉参差              | 快速测试、学习             |
...
1. 打开 [https://signup.cloud.oracle.com/](https://signup.cloud.oracle.com/) （或 [oracle.com/cn/cloud/free/](https://oracle.com/cn/cloud/free/)）
...
* 若提示 "Out of capacity"（ARM 无容量）：换可用域 / 区域，或退而创建 `VM.Standard.E2.1.Micro`（AMD 1 核 / 1GB，也够跑本项目）

1. **SSH 密钥**：上传本指南配套生成的公钥（见 1.3）

2. Boot volume 保持默认（50GB，在 200GB 免费配额内）
...
tar -czf deploy\free-server\manju-server.tar.gz --exclude="data/\*" --exclude="\_\_pycache\_\_" --exclude="\*.pyc" -C server .
...
ssh -i C:\Users\addk1\\.ssh\manju\_oci ubuntu@\<IP> "sudo mkdir -p /tmp/manju-deploy/keys && sudo chown -R ubuntu:ubuntu /tmp/manju-deploy"

\# 1) 代码包与部署脚本传到部署根目录

scp -i C:\Users\addk1\\.ssh\manju\_oci deploy\free-server\manju-server.tar.gz \`

&#x20;   deploy\free-server\install\_server.sh deploy\free-server\manju.service \`

&#x20;   deploy\free-server\fix\_ota\_url.py deploy\free-server\requirements.txt \`

&#x20;   ubuntu@\<IP>:/tmp/manju-deploy/

\# 2) OTA 签名密钥必须放到 keys/ 子目录（install\_server.sh 在此读取；

\#    私钥不上传仓库、仅在部署时直传服务器，且必须与客户端内置公钥同源）

scp -i C:\Users\addk1\\.ssh\manju\_oci keys\ota\_private.pem keys\ota\_public.pem \`
...
```
ssh -i C:\Users\addk1\\.ssh\manju\_oci ubuntu@\<IP> "sudo bash /tmp/manju-deploy/install\_server.sh http://\<IP>:8000"
```
...
```
cd clients/manju\_flutter

flutter run -d windows --dart-define=MANJU\_API\_BASE=http://\<IP>:8000
```
...
```
scp deploy\free-server\manju-server.tar.gz deploy\free-server\install\_server.sh \`

&#x20;   deploy\free-server\manju.service deploy\free-server\fix\_ota\_url.py \`

&#x20;   deploy\free-server\requirements.txt keys\ota\_private.pem keys\ota\_public.pem \`
...
1. 执行：`ssh root@<IP> "bash /tmp/manju-deploy/install_server.sh http://<IP>:8000"`
...
| ARM 无容量（Out of capacity）    | 换可用域（AD）或区域；或先建 E2.1.Micro（1 核 / 1G）                                              |
...
├── install\_server.sh   服务器一键部署脚本（无 Docker）

├── manju.service       systemd 服务单元

├── fix\_ota\_url.py      修正 OTA 产物 URL + 重新签名
